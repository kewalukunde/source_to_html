...
/********** state handlers **********/
//pevent_t * pstate_preprocessor_handler(FILE *fd, int ch);
...
static int is_pre_processor(char *word)
{
    int idx = 0;
    //printf("word = %s\n",word);
    ...
    switch(ch)
    {
        ...
        case '0' ... '9' : // detect numeric constant
            ...
            if (('A'-'Z' != ch) || ('a'-'z' != ch) || ('0'-'9' != ch))
            {
                ...
                else //numerical value begin
                {
#ifdef DEBUG	
                    printf("Numerical value Begin : \n");
#endif
        ...
        default : // Assuming common text starts by default.
            //printf("i come here in defiult ch: %c\n",ch);
            pevent_data.data[event_data_idx++] = ch;
            break;
    }
    return NULL;
...
pevent_t * pstate_sub_preprocessor_main_handler(FILE *fd, int ch)
{
    ...
    switch (ch)
    {
        ...
        case '"' : 
            fseek(fd, -1L, SEEK_CUR); // unget chars
            pevent_data.data[event_data_idx] = '\0';
            //printf("pevent_data : %s\n",pevent_data.data);
            status = is_pre_processor(pevent_data.data);
            //printf("status : %d\n",status);
            if (status == INCLUDE_HEADER)
            {
                //pevent_data.property = status;
                set_parser_event( PSTATE_IDLE,PEVENT_PREPROCESSOR_DIRECTIVE);
                return &pevent_data;
            }
            if (status == DEFINE_HEADER)
            {
...
pevent_t * pstate_ascii_char_handler(FILE *fd, int ch)
{
    ...
    switch(ch)
    {
        ...
        default :  // collect ascii char chars
            pevent_data.data[event_data_idx++] = ch;
            break;
    }
    return NULL;
